...
# Langfuse - это платформа для мониторинга, отладки и улучшения LLM приложений
# Она позволяет отслеживать все запросы к LLM, анализировать производительность,
# стоимость токенов, качество ответов и создавать аналитические дашборды
# Подробнее: https://langfuse.com/
try:
    langfuse_handler = CallbackHandler()
    langfuse_enabled = True
except Exception as e:
    print(f"⚠️  Langfuse не удалось инициализировать: {e}")
    print("   Возможные причины: отсутствие переменных LANGFUSE_SECRET_KEY, LANGFUSE_PUBLIC_KEY, LANGFUSE_HOST")
    print("   Приложение продолжит работу без трейсинга в Langfuse")
...
def main():
    ...
    if langfuse_enabled:
        ...
        print("   📊 Все запросы к LLM логируются для анализа производительности и стоимости")
        print("   🎯 Вы можете анализировать качество промптов и оптимизировать расход токенов\n")
    else:
        print("⚠️  Langfuse трейсинг отключен - данные о запросах не сохраняются")
        print("   💡 Для включения установите переменные: LANGFUSE_SECRET_KEY, LANGFUSE_PUBLIC_KEY, LANGFUSE_HOST\n")
    ...
    while True:
        ...
        while retry_count < max_retries:
            try:
                ...
                # Langfuse автоматически отслеживает: входной/выходной промпт, расход токенов,
                # время выполнения, стоимость запроса, версию модели и метрики качества
                # Все данные доступны в веб-интерфейсе Langfuse для аналитики и оптимизации
